...
const schema = [
  ...
  { type:'number', key:'exportSecs', label:'Export Length', min:1, max:3600, step:1, default:6, unit:'s' },
...
function render(){
  ...
  if (runMode === 'sweep'){
    ...
    if (!C.get('loop') && n > 1 && cycle >= n){ cycle = n - 1; pos = cycleLen; }

    // The sheet starts EMPTY. The first pass (cycle 0) lays image 0 tile by tile
    // onto bare paper; each later pass lays the next image over the one before.
    // The frozen base is the previous pass's image — nothing at all on the first
    // pass, so gaps show paper while image 0 assembles.
    const baseCycle = cycle - 1;
    const baseImg = baseCycle >= 0 ? ((baseCycle % n) + n) % n : -1;

    for (const b of blocks){
      if (!covered(b)) continue;
      const laid = b.rank < pos;
      // A laid tile carries this pass's image; the rest still show the previous
      // pass (nothing during cycle 0, so untouched tiles are still bare paper).
      const shownCycle = laid ? cycle : cycle - 1;
      const img = shownCycle >= 0 ? ((shownCycle % n) + n) % n : -1;
      if (img >= 0){ drawnThisFrame.push([b.rank, img]); if (trace) drawn.push([b.rank, img]); }
      ...
      for (const p of b.panels){
        ...
        // Both are skipped when there's nothing yet (index -1), leaving bare paper.
        if (basePeek && baseImg >= 0)
