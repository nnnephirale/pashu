...
# The demo photos and paper scans are identical across the sibling apps, so they
# live once at the project root. A clone in a subfolder has no assets/ of its
# own and falls back to the parent's shared copy — no duplicated binaries.
...
def link_list(dirname, pattern, urlprefix):
    """Hi-res build links assets by URL instead of embedding them, so editing a
    file on disk and refreshing the page shows the change immediately."""
    files = sorted(
        (f for f in os.listdir(dirname) if pattern.match(f)),
        key=lambda f: int(pattern.match(f).group(1)))
...
def main():
    ...
    # photos) by relative URL. The app is served with this folder as a subdir of
    # the repo root, so ../assets resolves to the shared asset folder — and any
    # edit to a PNG there shows up on the next page refresh.
