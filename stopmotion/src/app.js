...
const refreshStrips = () => {
  dirty = true;
  syncSaveBtn();
  // collageMode() turns on the moment the first image lands (and off when the
  // last one goes), so the mode locks have to follow the photo list too.
  syncCollageLocks();
...
// Cutout splits the app into two render paths, and each has controls the other
// ignores. In collage mode only the cut-out subject is drawn (sized by Subject
// Size); the full image pressed into the sheet — what Print Scale, Fit and the
// print blend act on — isn't drawn at all unless Image background brings it back.
// Whole-image mode is the reverse. Lock whichever set is inert so a dragged
// slider that does nothing says why instead of looking broken.
const PRINT_LOCK = 'Not used while Cut out subjects is on — the sheet shows the ' +
  'cut-out. Size it with Subject Size, or turn on Image background to print the ' +
  'full image behind it';
const SUBJECT_LOCK = 'Only used with Cut out subjects on';
function syncCollageLocks(){
  const collage = collageMode();
  const printInert = collage && !C.get('imageBg');
  for (const k of ['imageFit', 'printBlend', 'printOpacity', 'printScale'])
    C.setLocked(k, printInert, C.get(k), PRINT_LOCK);
  for (const k of ['subjectScale', 'subjectX', 'subjectY', 'edgeWidth', 'edgeRough', 'pieceShadow'])
    C.setLocked(k, !collage, C.get(k), SUBJECT_LOCK);
  dirty = true;
}
C.onChange('cutout', syncCollageLocks);
C.onChange('imageBg', syncCollageLocks);
syncCollageLocks();
